...
The project can be containerized and run with Docker Compose. This section describes how to build the Docker image, push it to Docker Hub, and launch the full stack.

### Prerequisites
- [Docker Desktop](https://www.docker.com/products/docker-desktop) installed and running.
- A Docker Hub account. Your username is **kpreslav**.

### Build the frontend image
```bash
# From the project root
docker build -t kpreslav/taski-frontend:latest .
```

### Push the image to Docker Hub
```bash
docker login   # use your Docker Hub credentials
docker push kpreslav/taski-frontend:latest
```

### Run the stack with Docker Compose
```bash
# The compose file will launch the frontend and the Appwrite backend.
# The backend image is pulled directly from Docker Hub.

docker compose up --build
```

The frontend will be available at **http://localhost** and the Appwrite backend at **http://localhost:8080**.

### Clean up
```bash
docker compose down   # stop containers
docker volume prune    # remove leftover volumes (optional)
```

---

*All Docker images are public and can be pulled with the commands above.*
